...
export interface TimelineEvent {id:string;type:TimelineType;entity_id:number;at:number;local_date:string;label:string;detail:string;weight_kg?:number|null;deleted:boolean}
...
export async function timeline(db:DbAdapter,start:string,end=start,includeUndo=false):Promise<TimelineEvent[]> {
  ...
  const rows=await db.all<TimelineEvent>(`
   SELECT 'meal:'||m.uuid id,'meal' type,m.id entity_id,m.logged_at at,m.local_date,
     COALESCE(m.meal_slot,'Meal') label,COALESCE((SELECT group_concat(display_name,', ') FROM log_items WHERE meal_id=m.id AND deleted_at IS NULL),'Analysis pending') detail,NULL weight_kg,m.deleted_at IS NOT NULL deleted FROM meals m
   UNION ALL SELECT 'weight:'||uuid,'weight',id,logged_at,local_date,'Bodyweight',weight_kg||' kg',weight_kg,deleted_at IS NOT NULL FROM weight_entries
   UNION ALL SELECT 'workout:'||uuid,'workout',id,COALESCE(finished_at,started_at),local_date,name,status,NULL,deleted_at IS NOT NULL FROM workouts
   UNION ALL SELECT 'exercise:'||uuid,'exercise',id,logged_at,local_date,name,kcal||' kcal activity (separate from food targets)',NULL,deleted_at IS NOT NULL FROM exercise_entries
   UNION ALL SELECT 'day:'||local_date,'day_status',CAST(replace(local_date,'-','') AS INTEGER),updated_at,local_date,'Day status',completion,NULL,0 FROM day_status
  `)
